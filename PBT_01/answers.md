...
- 3 thẻ semantic HTML5 mà trang đó sử dụng là:
    - Thẻ `<header>` dùng cho phần đầu chứa Logo,Menu ![alt text](../screenshots/image-2.png) ở phần tô xanh trong ảnh.
    - Thẻ `<footer>` dùng cho khu vực cuối trang ![alt text](../screenshots/image-3.png) ở phần tô xanh trong ảnh.
    - Thẻ `<nav>` dùng cho đường điều hướng ![alt text](../screenshots/image-4.png) ở phần to xanh trong ảnh

- 2 thẻ mà trang không dùng đúng semantic:
    - Lạm dụng thẻ `<div>` làm thẻ gốc thay vì dung `<main>` thì trang web dùng `<div id="main">`.![alt text](../screenshots/image-5.png)
    - Lỗi dùng `<section>` mà không có tiêu đề (`<h2>` đến `<h6>`) trang web chỉ dùng thẻ `<section>` đẻ làm khung chứ nút bấm mà có tiêu đề. 

2.table ![alt text](../screenshots/image-6.png)

- table hiển thị bảng quy dổi kích thước.
- Có trang web dùng cả `<thead>` và `<tbody>`.

3.form ![alt text](../screenshots/image-7.png)
- Như trong ảnh trên không có action và method.
- Input types được dùng là:
    - Thẻ `<input>` có thuộc tính role="combobox".![alt text](../screenshots/image-8.png)
    - Thẻ `<button type="button">` chứ biểu tượng tìm kiếm.![alt text](../screenshots/image-9.png)
